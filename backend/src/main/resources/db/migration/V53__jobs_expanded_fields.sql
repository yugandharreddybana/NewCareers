...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'career_operations' AND table_name = 'jobs' AND column_name = 'url') THEN
        ALTER TABLE jobs RENAME COLUMN url TO source_url;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'career_operations' AND table_name = 'jobs' AND column_name = 'source') THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
         WHERE conrelid = 'career_operations.jobs'::regclass
