...
export default function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className="fixed left-0 top-0 h-full bg-white border-r border-border flex flex-col z-30 overflow-hidden"
    >
      {/* Logo */}
      <div className="flex items-center h-[60px] px-4 border-b border-border shrink-0">
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.span
                ...
                className="font-bold text-[15px] tracking-tight text-text-primary whitespace-nowrap overflow-hidden font-display"
              >
                CareerOps
              </motion.span>
            )}
        ...
        <motion.button
          ...
          animate={{ marginLeft: collapsed ? 'auto' : 'auto' }}
          className="ml-auto shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-text-tertiary hover:text-text-secondary hover:bg-surface-overlay transition-colors"
        >
          {collapsed
  ...
  return (
    <Tooltip content={item.label} placement="right" disabled={!collapsed}>
      ...
      >
        {({ isActive }) => (
          <>
            <span className={cn('shrink-0 transition-colors', isActive ? 'text-brand-500' : 'text-text-tertiary group-hover:text-text-secondary')}>
              {item.icon}
            </span>
            <AnimatePresence>
              {!collapsed && (
