...
interface Props { data: OutreachData | null; open: boolean; onClose: () => void; }

export default function OutreachPanel({ data, open, onClose }: Props) {
  const [msg, setMsg] = useState<string | null>(null);

  useEffect(() => {
    if (msg === null && data?.body) {
      setMsg(data.body);
    }
  }, [data?.body, msg]);
    ...
    <SkillPanel title="Draft Outreach" open={open} onClose={onClose}>
      <div className="space-y-4">
        <div className="flex gap-2 items-center">
          ...
          {data.wordCount && data.wordCount > 0 && <span className="text-xs text-slate-400">{data.wordCount} words</span>}
        </div>
