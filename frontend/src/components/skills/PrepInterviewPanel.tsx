      ...
      <div className="space-y-5">
        ...
        {data.likelyQuestions?.length > 0 && (
          <Section title="Likely Questions">
            <div className="space-y-2">
              {data.likelyQuestions.map((q: any, i: number) => (
                <div key={i}
                  className="border border-slate-200 rounded-lg overflow-hidden cursor-pointer"
                  onClick={() => setExpanded(expanded === i ? null : i)}>
                  <div className="flex items-center gap-3 p-3">
                    <span className={`chip border ${CATEGORY_COLORS[q.category] || 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                      {q.category}
                    </span>
        ...
        {data.studyPlan?.length > 0 && (
          <Section title="Study Plan">
            <div className="space-y-2">
              {data.studyPlan.map((item: any, i: number) => (
        ...
        {data.redFlagsToAddress?.length > 0 && (
          <Section title="Red Flags to Address">
            <ul className="space-y-1">
              {data.redFlagsToAddress.map((f: string, i: number) => (
