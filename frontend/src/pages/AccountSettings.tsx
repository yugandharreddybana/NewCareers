...
function SettingsSavedSnapshot({ form }: { form: SettingsFormState }) {
  const workRows = form.workExperience.filter(
    w => w.jobTitle.trim() || w.companyName.trim(),
  );
  ...
  return (
    <div className="onboarding-panel">
      ...
      <div className="onboarding-grid-2">
        <div>
          <label className="settings-label" htmlFor={id('jobTitle')}>
            Job title
          </label>
          <input
            id={id('jobTitle')}
        ...
        <div>
          <label className="settings-label" htmlFor={id('company')}>
            Company
          </label>
          <input
            id={id('company')}
    ...
    <div className="onboarding-panel">
      ...
      <div className="onboarding-grid-2">
        <div className="sm:col-span-2">
          <label className="settings-label" htmlFor={id('school')}>
            School / university
          </label>
          <input
            id={id('school')}
        ...
        <div>
          <label className="settings-label" htmlFor={id('degree')}>
            Degree
          </label>
          <input
            id={id('degree')}
        ...
        <div>
          <label className="settings-label" htmlFor={id('field')}>
            Field of study
          </label>
          <input
            id={id('field')}
        ...
        <div>
          <label className="settings-label" htmlFor={id('year')}>
            Graduation year
          </label>
          <input
            id={id('year')}
    ...
    <div className="space-y-6">
      <div>
            ...
            .map(role => (
              <span key={role} className="settings-chip">
                ...
                <button
                  ...
                  onClick={() =>
                    patch({ selectedRoles: form.selectedRoles.filter(r => r !== role) })
                  }
                >
                  ×
                </button>
        ...
        <div className="flex gap-2 mt-3">
          ...
          <button
            type="button"
            className="px-4 py-2 rounded-lg border border-outline-variant text-primary font-label-md hover:bg-primary-fixed/20 transition-colors shrink-0"
            onClick={() => {
              const v = customRole.trim();
              if (!v) return;
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="settings-label" htmlFor="settings-salary-min">
              Minimum (k)
            </label>
            <input
              ...
              onChange={e =>
                patch({
                  salaryMinK: Math.min(Number(e.target.value) || 0, form.salaryMaxK),
                })
              }
            />
          </div>
          <div>
            <label className="settings-label" htmlFor="settings-salary-max">
              Maximum (k)
            </label>
            <input
              ...
              onChange={e =>
                patch({
                  salaryMaxK: Math.max(Number(e.target.value) || 0, form.salaryMinK),
                })
              }
            />
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="settings-label" htmlFor="settings-availability">
            Availability
          </label>
          <select
...
export default function AccountSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateProfile(settingsFormToPayload(form));
      ...
      <main className="flex-grow px-margin-mobile md:px-margin-desktop py-8 md:py-12 max-w-3xl mx-auto w-full">
        <header className="mb-8">
          ...
          <p className="font-body-md text-body-md text-on-surface-variant mt-2">
            Manage your profile, CV, experience, and job preferences.
          </p>
          {profileLoadFailed && (
        ...
        >
          ...
          <section className="settings-section">
            <SectionHeader
              icon="lock"
              title="Password"
              description="Reset your password via a secure link sent to your email."
            />
            <Link
              to="/forgot-password"
          ...
          <section className="settings-section">
            ...
            <div className="space-y-4">
              <div>
                <label className="settings-label" htmlFor="settings-headline">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="settings-label" htmlFor="settings-location">
                    Location
                  </label>
                  <input
                ...
                <div>
                  <label className="settings-label" htmlFor="settings-goal-location">
                ...
                <div>
                  <label className="settings-label" htmlFor="settings-exp">
                    Years of experience
                  </label>
                  <select
                    id="settings-exp"
          ...
          <section className="settings-section">
            ...
            <div className="space-y-4">
              {form.workExperience.map((entry, i) => (
                <WorkEntryCard
                  ...
                  onRemove={() =>
                    patch({
                      workExperience: form.workExperience.filter((_, j) => j !== i),
                    })
                  }
                />
              ))}
            ...
            <button
              type="button"
              onClick={() =>
                patch({ workExperience: [...form.workExperience, emptyWorkEntry()] })
              }
            ...
            <div className="space-y-4">
              {form.education.map((entry, i) => (
                <EducationEntryCard
                  ...
                  onRemove={() =>
                    patch({ education: form.education.filter((_, j) => j !== i) })
                  }
                />
              ))}
            </div>
